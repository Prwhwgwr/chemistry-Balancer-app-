...
# --- Hide Branding ---
# Adjusted CSS to be less aggressive to avoid potential layout rendering issues with tabs
hide_st_style = """
            <style>
            .stAppDeployButton, .stDeployButton, #MainMenu, [data-testid="stViewerBadge"], footer {display:none !important;}
...
if st.session_state.logged_in:
    # Explicitly using a container for tabs to ensure proper rendering
        ...
        with tab1:
            ...
            if st.button("Balance Equation"):
                if not st.session_state.is_premium and st.session_state.daily_balances >= 5:
                    st.error("🛑 Daily Limit Reached! Upgrade in the 'Help' tab.")
                else:
                    try:
                ...
                if code.strip() == "":
                    ...
                    with st.spinner("Checking database..."):
                        try:
                            ...
                            st.error("Database error. Make sure your secrets are configured!")
